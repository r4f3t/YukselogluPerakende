...
namespace B2BAdmin
{
    public partial class FrmKampRec : Form
    {
        public FrmKampRec()
        {
            InitializeComponent();
        ...
        private string SQLSTRYap()
        {
            ...
            //    if (CMBA19.Text == "<=") { ARAStr = "<="; BitSTR = ""; }
            //    if (CMBA19.Text == ">=") { ARAStr = ">="; BitSTR = ""; }
            //    //gruba göre okuma yapacak tek tek or ile yazacaksın
            //    // SqlDataReader oku2 = baglanti.DataReaderAl("select  from A_CYPHCODELine where VREF=" + CMBYetki.Text.Split('-')[1] + "");
        ...
        private void SLineGetir()
        {
            ...
            GridCLine.DataSource = baglanti.TabloAl("select INDORAN,VADEAD,MIKAD,A11,A14,A15,A16,A17,A18,A10,A12,A13,A19,LOGICALREF,CMBA10,CMBA11,CMBA12,CMBA13,CMBA14,CMBA15,CMBA16,CMBA17,CMBA18,CMBA19,YINDORAN from A_CMPGNLineRec where RecTanim='" + TXTCKod.Text + "'");
        ...
        string IISNEW;
        private void FrmKampRec_Load(object sender, EventArgs e)
        {
            IISNEW = FRMAna.ISNEW;
            //if (FRMCari.GFCad != null) { IISNEW = "0"; TXTCKod.Enabled = false; }
            TXTCKod.Text = FRMKampRecBas.GISTR;
        ...
        private void LOGOCampYap(string LClientref)
        {
        ...
        {
            if (TXTCKod.Text == "") { MessageBox.Show("KAmpanya Başlığı girmeden İşlem Yapamazsınız!!!"); TXTCKod.BackColor = Color.Red; return; }
            ...
            int i = 0;
                ...
                if (dialogResult == DialogResult.Yes)
                {
                    ...
                    string LCampaignRef="0";
                    if(CMBYetki.Text!="")
                    {
                    YetkiAd=CMBYetki.Text.Split('-')[0];
                    YetkiRef=CMBYetki.Text.Split('-')[1];
                    }
                    SqlDataReader oku = baglanti.DataReaderAl("select RecTanim from A_CMPGNLineRec where SQLSTR='" + SQlSTR + "' and RecTanim='"+TXTCKod.Text+"'"); if (oku.Read()) { MessageBox.Show("Bu Kampanya içinde Aynı Şartlardan Bulunmaktadır!!!");return; }
                    baglanti.VeriIslemler("INSERT INTO [A_CMPGNLineRec](YetkiAD,YetkiRef,YINDORAN,RecTanim,QLREF,QREF,[CLIENTREF],[SQLSTR],[INDORAN],[ADDDATE],[EDITDATE],VADEAD,MIKAD,VADEREF,MIKREF,A10,A11,A12,A13,A14,A15,A16,A17,A18,A19,CMBA10,CMBA11,CMBA12,CMBA13,CMBA14,CMBA15,CMBA16,CMBA17,CMBA18,CMBA19)VALUES" +
                    "('"+YetkiAd+"',"+YetkiRef+","+TXTYINDor.Text.Replace(",",".")+",'"+TXTCKod.Text+"'," + HFQLREF.Text + "," + HFQREF.Text + "," + CLIENTREF[i] + ",'" + SQlSTR + "'," + TXTCIndOr.Text.Replace(",", ".") + ",'" + Tarih + "','" + Tarih + "','" + CMBVade.Text + "','" + CMBMik.Text + "'," + CMBVade.Text.Split('-')[1] + "," + CMBMik.Text.Split('-')[1] + ",'" + TXTNAME.Text + "','" + TXTCODE.Text + "','" + TXTPRODUCERCODE.Text + "','" + TXTSTGRPCODE.Text + "','" + TXTSPECODE2.Text + "','" + TXTSPECODE3.Text + "','" + TXTSPECODE4.Text + "','" + TXTSPECODE5.Text + "','" + TXTSPECODE.Text + "','"+CMBYetki.Text+"','" + CMBA10.Text + "','" + CMBA11.Text + "','" + CMBA12.Text + "','" + CMBA13.Text + "','" + CMBA14.Text + "','" + CMBA15.Text + "','" + CMBA16.Text + "','" + CMBA17.Text + "','" + CMBA18.Text + "','"+CMBA19.Text+"')");
                    SqlDataReader oku4=baglanti.DataReaderAl("select LOGICALREF from A_CMPGNLineRec where RecTanim='"+TXTCKod.Text+"' and SQLSTR='"+SQlSTR+"'");
                    if (oku4.Read())
	                {
		                LCampaignRef=oku4[0].ToString();
	                }
                    //rectanima göre işlem yap
                

                    SLineGetir();
                    DialogResult dialogResult2 = MessageBox.Show("Şablon Satırının Kayıtlı olduğu \n Cariler Üzerinde Güncelleme Yapılacaktır ?", "", MessageBoxButtons.YesNo);
                    if (dialogResult2 == DialogResult.No) { return; }


                    SqlDataReader oku2 = baglanti.DataReaderAl("select CLIENTREF from A_CMPGNLine where RecTanim='" + TXTCKod.Text + "' group by CLIENTREF ");
                    while (oku2.Read())
                    {
                        string CLINEREFF = oku2[0].ToString();
                        //string LOGICALREF = oku2[1].ToString();
                       
                        SqlDataReader oku3 = baglanti.DataReaderAl("select RecTanim from A_CMPGNLine where SQLSTR='" + SQlSTR + "' and CLIENTREF=" + CLINEREFF + ""); if (oku3.Read()) { }
                        else
                        {
                            baglanti.VeriIslemler("INSERT INTO [A_CMPGNLine](CAMPAIGNREF,YetkiAD,YetkiRef,YINDORAN,RecTanim,QLREF,QREF,[CLIENTREF],[SQLSTR],[INDORAN],[ADDDATE],[EDITDATE],VADEAD,MIKAD,VADEREF,MIKREF,A10,A11,A12,A13,A14,A15,A16,A17,A18,A19,CMBA10,CMBA11,CMBA12,CMBA13,CMBA14,CMBA15,CMBA16,CMBA17,CMBA18,CMBA19)VALUES" +
                      "(" + LCampaignRef + ",'" + YetkiAd + "'," + YetkiRef + "," + TXTYINDor.Text.Replace(",", ".") + ",'" + TXTCKod.Text + "'," + HFQLREF.Text + "," + HFQREF.Text + "," + CLINEREFF + ",'" + SQlSTR + "'," + TXTCIndOr.Text.Replace(",", ".") + ",'" + Tarih + "','" + Tarih + "','" + CMBVade.Text + "','" + CMBMik.Text + "'," + CMBVade.Text.Split('-')[1] + "," + CMBMik.Text.Split('-')[1] + ",'" + TXTNAME.Text + "','" + TXTCODE.Text + "','" + TXTPRODUCERCODE.Text + "','" + TXTSTGRPCODE.Text + "','" + TXTSPECODE2.Text + "','" + TXTSPECODE3.Text + "','" + TXTSPECODE4.Text + "','" + TXTSPECODE5.Text + "','" + TXTSPECODE.Text + "','" + CMBYetki.Text + "','" + CMBA10.Text + "','" + CMBA11.Text + "','" + CMBA12.Text + "','" + CMBA13.Text + "','" + CMBA14.Text + "','" + CMBA15.Text + "','" + CMBA16.Text + "','" + CMBA17.Text + "','" + CMBA18.Text + "','" + CMBA19.Text + "')");
                            
                        }
                        LOGOCampYap(CLINEREFF);
                    }
            ...
            GRidIOCek();
            MessageBox.Show("Kampanya Kaydı Yapıldı");
        ...
        private void BTNSil_Click(object sender, EventArgs e)
        {
            ...
            if (dialogResult == DialogResult.Yes)
            {
                baglanti.VeriIslemler("delete from A_CMPGNLineRec where LOGICALREF=" + HFLineRef.Text + "");
                DialogResult dialogResult2 = MessageBox.Show("Şablon Satırının Kayıtlı olduğu \n Cariler Üzerinde Güncelleme Yapılacaktır ?", "", MessageBoxButtons.YesNo);
                if (dialogResult2 == DialogResult.No) { return; }

                

                SqlDataReader oku = baglanti.DataReaderAl("select CLIENTREF from A_CMPGNLine where CAMPAIGNREF="+HFLineRef.Text+" group by CLIENTREF ");
                while (oku.Read())
                {
                    string LCLIENTREF = oku[0].ToString();
                    baglanti.VeriIslemler("delete from A_CMPGNLine where CAMPAIGNREF=" + HFLineRef.Text + " and CLIENTREF="+LCLIENTREF+"");
                 
                    LOGOCampYap(LCLIENTREF);
                }
        ...
        private void GRidGezgin_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (GRidGezgin.AccessibleName == "SPECODE")
            {
                TXTSPECODE.Text = GRidGezgin.CurrentRow.Cells[0].Value.ToString();
                GRidIOCek();
            }
        }
        ...
        private void BTNUPD_Click(object sender, EventArgs e)
        {
            ...
            if (dialogResult == DialogResult.Yes)
            {
                ...
                baglanti.VeriIslemler("UPDATE [A_CMPGNLineRec]SET [SQLSTR] = '" + SQlSTR + "',[INDORAN] = " + TXTCIndOr.Text.Replace(",", ".") + ",[VADEAD] ='" + CMBVade.Text + "',[MIKAD] ='" + CMBMik.Text + "',[VADEREF] =" + CMBVade.Text.Split('-')[1] + "" +
   ...
   ",[RecTanim] ='" + TXTCKod.Text + "'" +
   ",[YINDORAN] ='" + TXTYINDor.Text.Replace(",", ".") + "'" +
   " WHERE CAMPAIGNREF=" + HFLineRef.Text + "");
                SqlDataReader oku = baglanti.DataReaderAl("select CLIENTREF from A_CMPGNLine where CAMPAIGNREF=" + HFLineRef.Text + " group by CLIENTREF ");
                while (oku.Read())
                {
                    string LCLIENTREF = oku[0].ToString();
        ...
        private void BTNDuzenBas_Click(object sender, EventArgs e)
        {
            baglanti.VeriIslemler("update A_CMPGNLineRec set RecTanim='"+TXTCKod.Text+"' where RecTanim='"+FRMKampRecBas.GISTR+"'");
            baglanti.VeriIslemler("update A_CMPGNLine set RecTanim='" + TXTCKod.Text + "' where RecTanim='" + FRMKampRecBas.GISTR + "'");
            MessageBox.Show("Değişim Yapıldı.");
        }
        ...
        private void TXTYINDor_TextChanged(object sender, EventArgs e)
        {
            if (TXTYINDor.Text=="")
            {
                TXTYINDor.Text = "0";
